...
#### 200427 Day 60 - 584~589p
서버 사이드 랜더링은 감만 잡은 상태다. 코드 스플리팅이랑 결합시키기 전에 538p부터 다시 볼 것.
redux-saga에는 END라는 특별한 action이 있다.이 액션을 실행하면 현재 진행중인 태스크들이 끝난 뒤, 사가가 하던 모든 액션 모니터링 작업을 종료한다.
```js
const serverRender = async (req, res, next) => {
const context = {};
